package com.kkapps.commons.views
...
import com.kkapps.commons.interfaces.RecyclerScrollCallback
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (dragSelectActive) {
                    ...
                    if (hotspotHeight > -1) {
                        if (ev.y >= hotspotTopBoundStart && ev.y <=hotspotTopBoundEnd) {
                            inBottomHotspot = false
                            if (!inTopHotspot) {
                                inTopHotspot = true
                                autoScrollHandler.removeCallbacks(autoScrollRunnable)
                        ...
                        } else if (ev.y >= hotspotBottomBoundStart && ev.y <= hotspotBottomBoundEnd) {
                            inTopHotspot = false
                            if (!inBottomHotspot) {
