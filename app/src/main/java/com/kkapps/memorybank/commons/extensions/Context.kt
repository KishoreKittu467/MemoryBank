...
import com.kkapps.memorybank.commons.helpers.MyContentProvider.Companion.COL_APP_ICON_COLOR
import com.kkapps.memorybank.commons.helpers.MyContentProvider.Companion.COL_BACKGROUND_COLOR
import com.kkapps.memorybank.commons.helpers.MyContentProvider.Companion.COL_LAST_UPDATED_TS
import com.kkapps.memorybank.commons.helpers.MyContentProvider.Companion.COL_NAVIGATION_BAR_COLOR
import com.kkapps.memorybank.commons.helpers.MyContentProvider.Companion.COL_PRIMARY_COLOR
import com.kkapps.memorybank.commons.helpers.MyContentProvider.Companion.COL_TEXT_COLOR
import com.kkapps.memorybank.commons.models.AlarmSound
import com.kkapps.memorybank.commons.models.BlockedNumber
import com.kkapps.memorybank.commons.models.SharedTheme
import com.kkapps.memorybank.commons.views.*
import java.io.File
...
fun Context.getFilePublicUri(file: File, applicationId: String): Uri {
    ...
    // if media content uri is null, get our custom uri like content://com.kkapps.memorybank.gallery.provider/external_files/emulated/0/DCIM/IMG_20171104_233915.jpg
...
fun Context.isThankYouInstalled() = isPackageInstalled("com.kkapps.memorybank.thankyou")

fun Context.isAProApp() = packageName.startsWith("com.kkapps.memorybank.") && packageName.removeSuffix(".debug").endsWith(".pro")
...
fun Context.getCanAppBeUpgraded() = proPackages.contains(baseConfig.appId.removeSuffix(".debug").removePrefix("com.kkapps.memorybank."))
...
fun Context.getDuration(path: String): Int? {
    ...
    return try {
        val retriever = MediaMetadataRetriever()
        retriever.setDataSource(path)
        val extraMeta = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DURATION)?.toInt()?:1
        (extraMeta / 1000f).roundToInt()
    } catch (ignored: Exception) {
        null
    }
}
...
fun Context.isDefaultDialer(): Boolean {
    return if (!packageName.startsWith("com.kkapps.memorybank.contacts") && !packageName.startsWith("com.kkapps.memorybank.dialer")) {
        true
    } else if ((packageName.startsWith("com.kkapps.memorybank.contacts") || packageName.startsWith("com.kkapps.memorybank.dialer")) && isQPlus()) {
